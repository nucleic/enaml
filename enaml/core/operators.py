...
class OpHandler(object):
    """ A class which manages reading and writing to operators.

    """
    __slots__ = ('read_ops', 'write_ops')

    def __init__(self, read_ops, write_ops):
        """ Initialize an OpHandler.

        Parameters
        ----------
        read_ops : sortedmap
            A sorted map of value-providing operators. The keys are
            the string names of the bound expressions and the values
            are callable objects which accept an owner and storage.

        write_ops : sortedmap
            A sorted map of change-accepting operators. The keys are
            the string names of the bound expressions, and the values
            are lists of callable objects which accept a change dict
            and storage.
    ...
    def read(self, name, owner, storage):
        """ Read the value from a bound operator.

        For performance reasons, this method assumes that there is a
        bound read operator for the given name.

        Parameters
        ----------
        name : str
            The name of the bound operator.

        owner : Declarative
            The declarative object for which the operator is being
            evaluated.
        ...
        """
        return self.read_operators[name](owner, storage)

    def write(self, change, storage):
        """ Write the change to a bound operator.

        For performance reasons, this method assumes that there are
        bound write operators for the given change.
        ...
        """
        for op in self.write_operators[change['name']]:
            op(change, storage)
...
class OperatorBase(object):
    ...
    """
    __slots__ = ('scopename', 'func')

    def __init__(self, scopename, func):
        """ Initialize an OperatorBase.

        Parameters
        ----------
        scopename : str
            The key name in the shared storage for the local scope
            to which the operator has access.

        func : FunctionType
            The function type implementing the op semantics.
    ...
    def __call__(self, owner, storage):
        ...
        This value must be implemented by subclasses.
...
class OpSimple(OperatorBase):
    ...
    def __call__(self, owner, storage):
        ...
        f_locals = storage.get(self.scopename) or {}
        func = self.func
        scope = DynamicScope(
...
class OpNotify(OperatorBase):
    ...
    def __call__(self, change, storage):
...
class SubscriptionObserver(object):
    ...
    """
    __slots__ = ('ownerref', 'name')

    def __init__(self, ownerref, name):
        """ Initialize a SubscriptionObserver.

        Parameters
        ----------
        ownerref : atomref
            An atomref to the declarative owner of interest.
