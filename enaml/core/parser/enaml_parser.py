...
class EnamlParser(Parser):
    ...
    def fstring_replacement_field(self) -> Optional[Any]:
        ...
        if (
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return self . make_fstring_replacement_field ( lbrace , debug_expr , ast . FormattedValue ( value = a , conversion = ( conversion if conversion else ( b'r' [0] if debug_expr and format is None else - 1 ) ) , format_spec = format , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset ) , );
        self._reset(mark)
        if (
            self.call_invalid_rules
            and
